...
public class BitefightChrome {
	...
	/** user inputs stored in <config.txt>
	 * @username -- account login name
	 * @password -- account login password
	 * @serverURL -- BiteFight server URL
	 * @stopGold -- bot pauses if Hero's gold amount reaches @stopGold threshold (0 = ignore it)
	 * @stopHP -- bot pauses if Hero's HP falls below @stopHP threshold
	 * @purchaseAttributes -- allow or disallow purchasing attributes
	 * @responseTime -- imitate human reaction time for clicking buttons, in milliseconds (minimum, maximum)
	...
	/** PROGRAM GLOBALS
	 * @storyActions -- array of indexed story actions
	 * @purchaseAttributeLimits -- array of maximum limits to purchase attributes (STR, DEF, DEX, END, CHA)
	 * @status -- array of Hero's status (gold, AP, HP)
	 * @attributes -- array of Hero's attribute basic values (STR, DEF, DEX, END, CHA)
	 * @isInStory -- determine if bot can use "Hunt" shortcut to enter story mode
	 */
	...
	private static void login(WebDriver driver) {
		...
		pause(300,500);
		driver.findElement(By.xpath("/html/body/div[4]/div[2]/div/div[2]/div/div/table/tbody/tr/td[2]/form/table/tbody/tr[1]/td[2]/input")).sendKeys(username);
		pause(300,500);
		driver.findElement(By.xpath("/html/body/div[4]/div[2]/div/div[2]/div/div/table/tbody/tr/td[2]/form/table/tbody/tr[2]/td[2]/input")).sendKeys(password);
		pause(300,500);
		driver.findElement(By.xpath("/html/body/div[4]/div[2]/div/div[2]/div/div/table/tbody/tr/td[2]/form/table/tbody/tr[5]/td[2]/input")).click();
	}

	/** RETRIEVE ACCOUNT INFO
	 * 
	 * @param overview -- click "Overview" button if page reload is required
	 * @param statusBool -- get Hero's status (gold, AP, HP) if required
	 * @param attributesBool -- get Hero's attributes basic values (STR, DEF, DEX, END, CHA) if required
	...
	private static void story(WebDriver driver) {
		// if bot has alerady entered story mode before, proceed to story mode via "Hunt" shortcut 
		if (isInStory == true) {
